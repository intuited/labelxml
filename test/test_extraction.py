...
class TextXPathGeneration(unittest.TestCase):
    def test_field_predicate(self):
        ...
        self.assertEqual(lxe.field_predicate_xpath(),
    ...
    def test_negated_predicates(self):
        self.assertEqual(lxe.field_predicate_xpath(negations=('price')),
                         './/text:p[@text:style-name="P10"]'
                            '[child::text()] '
    ...
    def test_template_predicate(self):
        self.assertEqual(lxe.template_xpath('{name} and {price}'),
                         './/text:p[@text:style-name="P10"]'
                            '[descendant::text()]'
                         ' and .//text:p[@text:style-name="P9"]'
                            '[descendant::text()]')
    ...
    def test_template_text_control(self):
        ...
        self.assertEqual(lxe.template_xpath(template,
...
class TestXPathValidity(LXMLBaseTestClass):
    def test_some_draw_frames_found(self):
        self.assertTrue(len(lxe.draw_frames(self.root)))
    def test_some_product_names_found(self):
        self.assertTrue(len(lxe.product_names(self.root)))
    def test_some_product_prices_found(self):
        self.assertTrue(len(lxe.product_prices(self.root)))

class TestXPathFirstElement(LXMLBaseTestClass):
    def test_product_name_of_first_draw_frame(self):
        df = lxe.draw_frames(self.root)[0]
        self.assertEqual(lxe.product_names(df)[0].text,
                         'Updated from Speerville price list, Fery 19th, 2009 ')
    def test_first_product_name(self):
        self.assertEqual(lxe.product_names(self.root)[0].text,
                         'Updated from Speerville price list, Fery 19th, 2009 ')
    def test_first_price(self):
        self.assertEqual(lxe.product_prices(self.root)[0].text,
...
class TestXPathLastElement(LXMLBaseTestClass):
    def test_product_name_of_last_draw_frame(self):
        ...
        df = lxe.draw_frames(self.root)[-1]
        self.assertEqual(lxe.product_names(df), [])
    def test_last_product_name(self):
        self.assertEqual(lxe.product_names(self.root)[-1].text,
                         'Red Quinoa')
    def test_last_price(self):
        self.assertEqual(lxe.product_prices(self.root)[-1].text,
                         '$8.00')
...
class TestElementCounts(LXMLBaseTestClass):
    def test_draw_frame_count(self):
        ...
        """
        self.assertEqual(len(lxe.draw_frames(self.root)),
                         169)
...
class TestPage297(LXMLBaseTestClass):
    def setUp(self):
        ...
        self.page = self.root.xpath(
            lxe.template_xpath('//draw:frame[@text:anchor-page-number="297"]'),
    ...
    def test_descendant(self):
        ...
        """
        descentext = lxe.xpath_text_formats('descendant')
        self.assertEqual(
            lxe.frame_data(self.page[0],
    ...
    def test_descendant_or_self(self):
        ...
        descentext = lxe.xpath_text_formats('descendant-or-self')
        self.assertEqual(
            lxe.frame_data(self.page[0],
                          text_formats=descentext)['name'],
            "Spelt Bran Bread")
    ...
    def test_child(self):
        ...
        tf = lxe.xpath_text_formats('child')
        self.assertEqual(
            lxe.frame_data(self.page[0],
